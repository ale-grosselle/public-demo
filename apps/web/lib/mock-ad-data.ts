...
const categories = [
  { category: 'Elettronica', subcategories: ['Telefoni', 'Computer', 'TV e Audio', 'Console'] },
  {
    category: 'Casa e Giardino',
    subcategories: ['Arredamento', 'Elettrodomestici', 'Decorazioni'],
  },
  { category: 'Auto e Moto', subcategories: ['Auto usate', 'Moto', 'Accessori auto'] },
  { category: 'Sport e Hobby', subcategories: ['Biciclette', 'Palestra', 'Calcio', 'Tennis'] },
  { category: 'Immobiliare', subcategories: ['Vendita', 'Affitto', 'Commerciale'] },
]

const conditions = ['Nuovo', 'Come nuovo', 'Buone condizioni', 'Discreto', 'Da riparare']
...
function generateImageUrls(id: string, count: number): string[] {
  return Array.from({ length: count }, (_, i) => `https://picsum.photos/800/600?random=${id}-${i}`)
}
...
export function generateMockAdData(id: string): AdData {
  ...
  const subcategory = getRandomFromArray(categoryData.subcategories, seedBase + 4)
  ...
  const daysAgo = Math.floor(seededRandom(seedBase + 10) * 30) + (numericId % 30)
  ...
  if (categoryData.category === 'Elettronica') {
    specifications['Marca'] = getRandomFromArray(['Apple', 'Samsung', 'Sony', 'LG'], seedBase + 20)
